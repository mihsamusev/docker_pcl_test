...
### Run the container
If you are on Windows start the container by running ``./start_pcl_shell.sh`` shell script in terminal or ``start_pcl_cmd`` if you prefer the command prompt. The script tells docker to run the
